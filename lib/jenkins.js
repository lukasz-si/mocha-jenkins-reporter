...
function Jenkins(runner, options) {
    ...
    options.junit_report_filename = process.env.JUNIT_REPORT_FILENAME || options.junit_report_filename || 'report';
    ...
    options.diff_images_path = options.diff_images_path || null;
            ...
            if (test.state == "failed") {
                ...
                if (options.screenshots) {
                    ...
                    if (options.screenshots == 'loop') {
                        ...
                        screenshot = path.join(screenshotDir, formatImageName(imagestring +
                    ...
                    browser.takeScreenshot().then(function (png) {
                        fsExtra.outputFile(screenshot, new Buffer(png, 'base64'));
                    });
                }
    ...
    function genDiffImagesReport() {
        if (options.diff_images_path) {
            var files = fs.readdirSync(options.diff_images_path).sort();

            if (files.length > 0) {
                writeString('<testsuite name="Diff images" package="screenshots.comparison.diffs">\n');
            }

            files.forEach(function (file) {
                var screenshotPath = path.join(process.cwd(), options.diff_images_path, file);

                writeString('<testcase classname="Diff images" name="Diff image of: ' + htmlEscape(file) + '">\n');

                writeString('<system-out>\n');
                writeString('[[ATTACHMENT|' + screenshotPath + ']]\n');
                writeString('</system-out>\n');
    ...
    /**
     * Convert whitespaces and non-word characters to "_"
     * @param name
    ...
    runner.on('start', function () {
        ...
        var fileName = options.junit_report_filename;
        if (reportPath) {
            if (fs.existsSync(reportPath)) {
                var isDirectory = fs.statSync(reportPath).isDirectory();
                if (isDirectory) reportPath = path.join(reportPath, fileName + ".xml");
            } else {
                mkdirp.sync(reportPath);
                reportPath = path.join(reportPath, fileName + ".xml");
            }
