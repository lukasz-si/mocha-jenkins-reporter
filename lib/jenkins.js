    ...
    function genSuiteReport() {
        ...
        tests.forEach(function (test) {
            ...
            if (test.state == "failed") {
                errMessage = test.err && test.err.message ? test.err.message : null;

                writeString('<failure message="');
                if (errMessage) {
                    writeString(htmlEscape(errMessage));
                }
                writeString('">\n');
                ...
                if (options.screenshots) {
                    ...
                    if (options.diff_images_path && errMessage) {
                        var files = fs.readdirSync(options.diff_images_path).sort();

                        files.forEach(function (file) {
                            if (errMessage.indexOf(file) !== -1) {
